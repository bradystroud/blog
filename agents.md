...
```bash
# Development
pnpm dev                    # Start dev server with TinaCMS at localhost:3000
pnpm build                  # Full production build (TinaCMS + Next + RSS + sitemap)
...
# Content Generation
pnpm generate-rss           # Generate RSS feed from blog posts
...
- ❌ Use `npm` or `yarn` (pnpm required - enforced by preinstall script)
