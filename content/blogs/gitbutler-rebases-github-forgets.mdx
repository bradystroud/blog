...
Forking was off the table too. GitButler is FSL-1.1-MIT, which lets me modify it privately but not publish a competing version until the change date - 2028 for the current CLI. And building a lane engine from scratch means reimplementing hunk ownership, the workspace merge commit, a rebase engine that carries conflicts forward, and the oplog. That's a lot of work to fix one push behaviour.
