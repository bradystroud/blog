...
const formatMarkdownToHTML = (markdown: string): string => {
  ...
  return (
    markdown
      ...
      .replace(
        /\[([^\]]*)\]\(([^)]*)\)/gim,
        '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 underline">$1</a>'
      )
      // Lists (basic support)
...
export default function AIBlog({ title, globalData }: AIBlogProps) {
  ...
  const setClientCache = (title: string, data: Record<string, unknown>) => {
    try {
      const cacheKey = getCacheKey(title);
      const cacheData = {
        ...data,
  ...
  return (
    <Layout data={globalData}>
      <Head>
        ...
        <meta
          name="description"
          content={`AI-generated content about ${formattedTitle.toLowerCase()}`}
        />
      </Head>
      <Section className="flex-1">
        <Container width="small" className="flex-1 pb-2" size="large">
                ...
                <div className="flex flex-col">
                  {isFallback ? (
                    <p className="text-sm text-yellow-700 dark:text-yellow-300">
                      <strong>Blog Post Not Found:</strong> This content doesn't
                      exist yet, but I've provided some information that might
                      be helpful. If you'd like me to write about this topic,
                      let me know!
                    </p>
                  ) : (
                    <p className="text-sm text-blue-700 dark:text-blue-300">
                      <strong>AI-Generated Content:</strong> This blog post was
                      automatically generated based on your search. While I
                      don't have a specific post on this topic yet, I've created
                      this content to help answer your questions.
                    </p>
                  )}
                  {isCached && (
